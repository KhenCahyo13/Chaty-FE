...
const ChatBox: FC<ChatBoxProps> = ({
    ...
    <div className="relative z-20 shrink-0 border-t border-border/70 bg-background/85 backdrop-blur">
        <form
            className="mx-auto flex max-w-5xl items-center gap-x-2 py-2 md:gap-x-3 md:py-2.5"
            onSubmit={(e) => {
                e.preventDefault();
                form.handleSubmit();
            }}
        >
            <Button
                ...
                variant='ghost'
                size='icon'
                className="rounded-xl border border-transparent hover:border-border/70 hover:bg-white"
            >
                <IconPlus className="size-5 text-muted-foreground" />
            </Button>
            <div className="flex flex-1 rounded-2xl border border-border/70 bg-white px-1 shadow-[0_20px_26px_-30px_oklch(0.28_0.04_250)]">
                <TfTextInput
                    form={form}
